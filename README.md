...
- **postgres-airflow** — отдельная PostgreSQL-база для служебных (metadata) данных Airflow: история запусков, статусы задач, пользователи веб-интерфейса. Не путать с `postgres-dwh`. Порт наружу не публикуется — доступна только внутри docker-сети. Для неё настроен `healthcheck` (`pg_isready`) — без него при самом первом запуске на чистом volume `airflow-init` мог упасть с ошибкой подключения, потому что Postgres ещё не успевал завершить инициализацию (`initdb`), а Compose по умолчанию ждёт только старта контейнера, а не его реальной готовности.
